...
m - queue of available gather targets (mineral patches and vespene geysers)
n - current used supply
o - insert a single mineral patch or vespene geyser into the queue
...
#define j(z)(unsigned char)a[22*((X->mapHash()[9]-4)%12+(D.x-4)%7)+z]-128
...
// Using characters in the extended ASCII range, we just add 128 to each tile position component and put it in a string
...
// Second vector is what to build, as an index to the unit types vector subtracted by 100
A<W>b = { 9, 15, 15, 15, 37, 32, 32, 51, 37, 61, 99, /*offset->*/ 35, 51, 71, 73, 51, 75 },
c = { 3,  5,  4,  3,  7,  8,  8,  3,  9,  0,  3, /*offset->*/  3,  3,  3,  3,  3 };
// Depot->Rax->Refinery->Depot->Factory->Port->Port->Depot->Tower->CC->Depot...
...
struct ExampleAIModule :AIModule {
	N onFrame() {
		...
		// Removes targets from the queue that are not occupied by an enemy building
		for (auto u = h.begin(); u != h.end();)
			u = X->isVisible(*u) && X->getUnitsOnTile(*u, B(Building) && B(Enemy)).empty() ? i[*u] = t, h.erase(u) : u + 1;

		// Adds any visible enemy buildings that are not already in the queue
		for (U u : X->enemy()->getUnits())
			!i[P] && Q.isBuilding() ? i[P] = u, h.push_back(P) : a;

		// Update the next building to construct
		// Times out a pending building if we've been waiting 240 frames to build it
		if (d&&L(l[f]) || O - d > 240)
			d = 0;

		// Checks if there is a building in our build order we want to build now
		for (; !G&&R < 16; R++)
			if (!d && !L(l[k(R)]) && O > b[R] * 99)
				//g(c[R] + 100, k(R));
				g(c[R] + 100, X->getBuildLocation(F[c[R] + 100], D));

		// Queues a supplyProviderBuilding if we're about to be supply blocked or a barracks if we have money for it
		if (!d&&w < 400)1.0*n / w > 0.8 ? g(103) : a, C->minerals() > 200 && x < 1 ? g(105) : a;

		w = n = x = R = 0; // supplyMax = supplyUsed = # of gateWays = buildOrder #
		for (U u : C->getUnits()) {
			...
			M(Q.isBuilding()) {
				l[P] = u; // register TilePosition of the building onto the map l
				...
				if ((!d || e % 3 != 0) && !u->isTraining()) {
					m.size() ? u->train(F[13]) : a;
					u->train(C->gas() < 50 ? F[4] : F[7]);
				}

				// Maybe construct add-ons here too?
