...
A<W>b={12,17,31,38,56,67,78,85,94},c={3,5,4,3,7,8,3,10,14};A<T>h;map<T,U>i,l;map<U,U>z,q;map<U,W>y;W d,n,w,R,O,e,cc,cd,cy;char G;T f,D,k9;U v,t;A<U>m;T k(W u) {RT T(j(u*2),j(u*2+1));}
...
struct ExampleAIModule :AIModule {
 N onFrame() {
  ...
  if (CC(104)==1&&GR(S(D),400,B(ResourceContainer)&&!BM).size()>1) BB(104);
 ...
 for (U u:C->getUnits()) {
  ...
  if (!u->isTraining()) {
   if (!d||e%3!=0) {
    if ((P==D||P==k9) && GR(S(P),320,BO&&BW).size()<21) ut(F[13]);
    if (CC(2)<12)ut(F[2]);}
   if (!CC(3)&&!CI(3)) ut(F[3]);
  G?ut(F[0]):ut(F[15]);}
  if (Q == F[100]) {
