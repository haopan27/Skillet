...
char*a = "k-k/},k2q5}.w5z8uW1’1Ž “1‹+ˆ ‘%‰ ‡'hl—l’l™kŽeŒ_˜_Ž]‘]xR%R)R#R,X-a%^-b+[Fmbh[mXukuov[ookouJ/‘2Š'Š4›8›'Œ>›>˜'z/12*/',>&>'* > ;D&“*Ž#Ž!Š,Š)“,Š&•.u&”™Œ’‘‘Œ›Œ˜”›Œ•™Žs–(™.‘+‘0š0—(›(Œ$ŒF–)*/\",\"1+1(),).#.H&s)s+s1s.y/€,€/Ž$VB•|‘t•~Œ}ŠzŒtŠw˜Ž}W;”;‘;Œ;ŽAŽG’HŒ-˜f|(E,=(G0F0C0=0@$/?g";
W CC(W u) {RT C->completedUnitCount(F[u]);}
W CI(W u) {RT C->incompleteUnitCount(F[u]);}
A<W>b={12,16,32,48,58,68,74,78},c={3,5,3,7,8,3,10,14};A<T>h;map<T,U>i,l;map<U,U>z,q;map<U,W>y;W d,n,w,R,O,e,cc,cd,cy;char G;T f,D,kn;U v,t;A<U>m;T k(W u) {RT T(j(u*2),j(u*2+1));}
...
struct ExampleAIModule :AIModule {
 N onFrame() {
	 O = X->getFrameCount(); kn = k(8);G?0:c[7]=3;if (!O) { D = C->getStartLocation(); H(i, read, &G); for (T u : X->getStartLocations())u != D ? h.push_back(u) : a; p(kn); p(D); }
  ...
  if (!d&&w<400&&CC(110)) 1.0*n/w>0.8?g(103):(CC(108)<3?g(108):(CC(114)&&CC(116)<6?g(116):a));
  if (n>21&&CC(104)<(int)GR(S(D),400,B(ResourceContainer)&&!BM).size()) BB(104);
  M(n>37&&CC(100)==1) BB(100); M(CC(104)==1&&!GR(S(kn),400,BO&&BW&&B(Completed)).empty()) BB(104,kn);
  w=n=R=0;
 for (U u:C->getUnits()) {
  ...
  if (!u->isTraining()) {
   if (!d||e%3!=0) {
    if ((P==D||P==kn) && GR(S(P),320,BO&&BW).size()<21) ut(F[13]);
    if (CC(2)<12)ut(F[2]);}
   if (!CC(3)&&!CI(3)) ut(F[3]);
  G?ut(F[0]):ut(F[15]);}
  if (Q == F[100]) {
