...
N BB(W u, T b0 = D, T z = D) { // b0: build location search center; z: builder search center
	if (O - cc > (b0 == kn ? 600 : 240)) {
		...
		L(br) ? br->build(F[u], u == 135 ? FindBuildingPlacementAroundAPoint(F[u], b0) : GB(F[u], b0, 18)) : "a";
	}
	...
	N onFrame() {
		...
		for (auto x = 0; x < Broodwar->mapWidth(); x++) {
			for (auto y = 0; y < Broodwar->mapHeight(); y++) {
				...
				if (bwemMapInstance.GetArea(t) == mouseArea)
		...
		if (!meCOEP) {
			...
			M(me4or5Pool) {
				...
				M (CL(123) < 4 && C->minerals() > 450) BB(123); // More hatches when possible when me4or5Pool
			}
			M(!me4or5Pool && my3rdBuilt && CC(123) < 6 && !countMyMorphingUnits(Zerg_Hatchery) && C->minerals() > 450) {
			...
			M(O >= 34560 && GR(S(D), 320, BM).empty() && !GR(S(D), 320, BO&&BW).empty()) BB(135);
		}
		M(meCOEP) {
		...
		for (U u : C->getUnits()) {
			...
			M(Q == F[34]) {
				if (!meCOEP) {
					...
					for (S ip : {S(D), S(kn)}) 
						if (ud(ip) < 128 && (W)GR(ip, 320, BO&&BW).size() < (me4or5PoolEarly ? bG[1] : 18) && CL(40) < (me4or5PoolEarly ? bG[1] : (myNatBuilt?36:18)))
							ut(F[40]); // Drones
